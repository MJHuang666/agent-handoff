...
3. 把共享 Skill 安装到项目根 `.agents/skills/project-role-workflow/`，把共享状态模板安装到 `docs/agent/`。如果宿主策略禁止写入 `.agents/`，继续使用已加载的全局 Skill 和 `docs/agent/protocol.md`，并报告项目级 Skill 为 pending；不要把它当作安装失败。
...
7. 安装后检查 Markdown 本地链接和 Skill frontmatter；再启动或指导用户启动一个全新会话，只输入“继续”进行真实入口验证。
