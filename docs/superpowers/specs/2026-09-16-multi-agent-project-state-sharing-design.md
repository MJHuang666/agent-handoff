...
版本：1.2.0，2026-09-16。本文记录已实现的设计基线：身份、活动任务、交接、检查点、恢复、语言选择与子代理门禁均已落入模板。结构、初始化和关键行为已做本地验证；没有经过全新工具会话验证的第三方适配仍必须标为未验证。
...
8. 纯 Markdown。第一版不包含 Python、Shell、Node.js 或其他自动化脚本。
...
第一版不实现自动编排器、后台进程、文件锁、数据库、网络同步、自动调用不同 Agent、自动提交 Git、任务看板 UI 或第三方项目管理平台集成。仓库可使用只读 CI 校验源码和发布包，但 CI 不属于项目运行时，也不改变纯文件协作协议。
