...
接受“更换 Agent”“替换 Agent”“replace agent”和“switch agent”。旧 Agent 或新 Agent 都可以发起这一明确授权的管理操作。角色保持不变，每次选择 `current-task`、`project-default` 或 `both` 范围，并创建或复用同角色身份。优先使用 `.agents/skills/agent-relay/scripts/workflow_state.py` 提供短时本地锁、expected revision 校验和原子替换。
