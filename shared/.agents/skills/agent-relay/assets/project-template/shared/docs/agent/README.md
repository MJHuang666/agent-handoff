...
旧 Agent 或新 Agent 都可以说 `$agent-relay 更换 Agent`、`替换 Agent`、`replace agent` 或 `switch agent`。工作流会依次选择角色、影响范围（当前任务/未来默认/两者）及同角色的新 participant，并在确认后调用标准库 Python 辅助脚本完成短时文件锁、revision 校验和原子写入。
