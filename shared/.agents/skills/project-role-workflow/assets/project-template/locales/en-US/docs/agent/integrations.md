...
| DeepSeek Harness | Yes | Root `AGENTS.md` + `.agents/skills/project-role-workflow/`; enable `dsh-agent-instructions` and filesystem skill loading | Yes | Static verification complete; fresh session pending | Local source / 2026-09-17 |
| OpenCode | Yes | Root `AGENTS.md` + `.agents/skills/project-role-workflow/` | Yes | Verified | 1.3.17 / 2026-09-17 |
...
DeepSeek Harness and OpenCode share `.agents/skills/project-role-workflow/` as the single source of truth. Do not create duplicate `.dsh/skills` or `.opencode/skills` trees. If OpenCode does not discover the skill, check skill permissions and higher-priority same-name copies.
