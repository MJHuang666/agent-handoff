...
Accept `replace agent`, `switch agent`, and the documented Chinese aliases. Either old or new Agent may start this explicit management operation. Keep the role fixed and choose `current-task`, `project-default`, or `both`; create or reuse a same-role identity. Prefer `.agents/skills/project-role-workflow/scripts/workflow_state.py` for its short-lived local lock, expected-revision validation, and atomic replacement.
