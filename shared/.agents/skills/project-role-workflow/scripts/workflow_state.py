#!/usr/bin/env python3
"""Safe state mutations for Project Role Workflow repositories.
...
def lock_path(repo: Path) -> Path:
    git_path = repo / ".git"
    if git_path.is_dir():
        return git_path / "project-role-workflow.lock"
    if git_path.is_file():
        ...
        if first_line.startswith("gitdir:"):
            ...
            return resolved_git_dir / "project-role-workflow.lock"
    return repo / ".project-role-workflow.lock"
...
def command_status(repo: Path) -> dict:
    ...
    payload = {
        ...
        "lock_present": lock_path(repo).exists(),
    }
...
def replace_agent(repo: Path, args: argparse.Namespace) -> dict:
    ...
    with state_lock(repo, "replace-agent") as transaction:
        ...
        return {
            ...
            "next_command": "$project-role-workflow continue",
        }
...
def main() -> int:
    ...
    try:
        if args.command == "status":
            ...
            path = lock_path(repo)
            if not path.exists():
                raise WorkflowError(f"no lock exists: {path}")
